...
def call_glm_translate(url: str, api_key: str, model: str, prompt: str, temperature: float, max_tokens: int) -> tuple[str, dict]:
    """调用 GLM API 进行翻译"""
    payload = {
        "model": model,
        "messages": [
...
def main() -> int:
    """主函数"""
    parser = argparse.ArgumentParser(description="使用 GLM API 翻译学术论文")
    parser.add_argument("--input", required=True, help="输入论文文件路径")
    parser.add_argument("--output", required=True, help="输出翻译文件路径")
    parser.add_argument("--model", default="glm-4-plus", help="GLM 模型名称")
    ...
    print(f"开始翻译论文: {input_path.name}")
    ...
    api_key = "5d706067fdb341718905a465e82331cc.1Cukv0fvO0byQmpp"  # GLM API key

    if args.dry_run:
        print("干运行模式 - 不会实际调用 API", flush=True)
        sys.stdout.flush()
        print("将要翻译 " + str(total_sections) + " 个章节", flush=True)
        return 0

    # 创建事件文件
    event_file = wrapper_event_path("glm_translate")
    ...
    for i, section in enumerate(sections, 1):
        ...
        try:
            ...
            translated, usage = call_glm_translate(
                api_base,
                api_key,
                args.model,
                prompt,
