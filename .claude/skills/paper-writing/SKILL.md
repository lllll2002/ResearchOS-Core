---
name: paper-writing
description: Assist with writing sections of a research paper (Introduction, Related Work, Method, Results, Discussion, Conclusion). Leverages workspace papers for citations and evidence. Use when the user wants help drafting or revising specific paper sections. For orchestrated multi-section writing with progress tracking and quality gates, use /paper-pipeline instead.
---
...
- **目标期刊/会议**：格式要求、篇幅限制
- **语言**：默认先写英文版，再翻译中文版；除非用户明确只要一种语言
...
2. 引用工作区论文建立研究脉络：
   ...
   scholaraio show <dir_name> --level 2      # 摘要
   ```
...
2. 从工作区论文中找到可对比的方法：
   ...
   scholaraio show <dir_name> --level 4      # 读全文了解方法细节
   ```
3. 确保符号定义一致、公式推导完整
...
2. 从工作区中检索可对比的基线结果：
   ...
   scholaraio show <dir_name> --level 3      # 结论
   ```
...
- 每个主要章节至少设计 1 张概念示意图或汇总表格
- 图表在 Markdown 中用表格、ASCII 图或 Mermaid 图语法呈现
- **每张图/表必须有独立完整的图注（caption）**，读者不看正文也能理解：
  ```
  **Figure N.** Caption describing what is shown, key findings, and data source.
  **Table N.** Caption describing table content and scope.
  ```
- 中文版图注保持与英文版编号一致，单独翻译

## 双语输出规则

1. 默认先写**英文版**，保存为 `workspace/<name>/<section>-en.md`
2. 完成后翻译**中文版**，保存为 `workspace/<name>/<section>-zh.md`
3. 用户明确只要一种语言时可跳过另一版
...
3. **图表质量**：每张图表是否有图注？是否在正文中被引用？图例、坐标轴标签是否完整？
