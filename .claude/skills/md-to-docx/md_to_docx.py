    ...
    def parse_inline(text: str) -> List[Dict[str, Any]]:
        ...
        while i < len(text):
            if text[i] == '`' and (i == 0 or text[i-1] != '\\'):
                ...
                # Citation superscript: ^12^ or ^1,2,3^ → [12] as superscript
                end_idx = text.find('^', i + 1)
                if end_idx != -1:
                    inner = text[i+1:end_idx]
                    # Only treat as citation if content is digits/commas/hyphens
                    if re.match(r'^[\d,\-–\s]+$', inner):
                        parts.append({'type': 'superscript', 'content': f'[{inner}]'})
                        i = end_idx + 1
                    else:
                        parts.append({'type': 'text', 'content': text[i]})
                        i += 1
                ...
                if next_sup != -1:
                    candidates.append(('superscript', next_sup))
...
class DocxGenerator:
    ...
    def _add_formatted_text(self, paragraph, text: str, base_size: float = 12):
        ...
        for part in parts:
            ...
            elif part['type'] == 'superscript':
                run = paragraph.add_run(part['content'])
                run.font.size = Pt(base_size * 0.75)
                run.font.superscript = True
                self._set_run_font(run, size=base_size * 0.75)
            elif part['type'] == 'br':
