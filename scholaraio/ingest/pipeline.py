...
def step_dedup(ctx: InboxCtx) -> StepResult:
    ...
    if not doi or not doi.strip():
        ...
        if _detect_thesis(ctx):
            ...
            return StepResult.OK
        # Not thesis -> text-based duplicate check before moving to pending
        dup_label = _text_duplicate_check(ctx)
        if dup_label:
            ui(f"文本相似度重复（LCS≥50），视为同一论文: {dup_label}")
            _move_to_pending(ctx,
                             issue="duplicate",
                             message="无 DOI，但前100词与已有论文高度相似（LCS≥50），视为重复",
                             extra={"duplicate_of": dup_label, "method": "text_lcs"})
...
def step_ws_assign(papers_dir: Path, cfg: Config, opts: dict[str, Any]) -> None:
    """Auto-assign newly ingested papers to workspaces via LLM."""
    from scholaraio.workspace import auto_assign
    from scholaraio.papers import iter_paper_dirs, read_meta

    ws_root = papers_dir.parent.parent / "workspace"
    if not ws_root.exists():
        return

    # Only process papers ingested in this run (check by recent mtime)
    import time
    ...
    for pdir in iter_paper_dirs(papers_dir):
        ...
        added = auto_assign(paper_id, title, abstract, ws_root, cfg.index_db, cfg)
        if added:
            assigned_count += 1
            _log.info("  %s -> %s", title[:50], ", ".join(added))

    if assigned_count:
        _log.info("Auto-assigned %d papers to workspaces.", assigned_count)


def step_chunks(papers_dir: Path, cfg: Config, opts: dict[str, Any]) -> None:
    """Build section-level chunks for papers with TOC."""
    from scholaraio.vectors import build_chunks

    n = build_chunks(papers_dir, cfg.index_db, rebuild=False, cfg=cfg)
    if n:
        _log.info("Chunks done, %d new.", n)
    else:
        _log.info("Chunks up to date.")
...
STEPS: dict[str, StepDef] = {
    ...
    "embed":      StepDef(fn=step_embed,      scope="global", desc="Build paper-level vectors"),
    "chunks":     StepDef(fn=step_chunks,    scope="global", desc="Build section-level chunks"),
    "ws_assign":  StepDef(fn=step_ws_assign, scope="global", desc="Auto-assign papers to workspaces (LLM)"),
    "index":      StepDef(fn=step_index,     scope="global", desc="Update FTS5 index"),
}

PRESETS: dict[str, list[str]] = {
    "full":    ["mineru", "extract", "dedup", "ingest", "toc", "l3", "embed", "chunks", "ws_assign", "index"],
    "ingest":  ["mineru", "extract", "dedup", "ingest", "embed", "chunks", "ws_assign", "index"],
    "enrich":  ["toc", "l3", "embed", "chunks", "index"],
    "reindex": ["embed", "index"],
}
    ...
    *,
    ...
    if use_cloud_batch and needs_mineru and not dry_run:
        ...
        if pdfs_to_convert:
            ...
            from scholaraio.ingest.mineru import _safe_data_id
            for br in batch_results:
                pdf_stem = br.pdf_path.stem
                # MinerU names the images folder using safe_data_id (non-ASCII replaced with _)
                safe_stem = _safe_data_id(pdf_stem)
                namespaced_images = inbox_dir / f"{safe_stem}_images"
                if namespaced_images.is_dir():
                    target = inbox_dir / f"{pdf_stem}_mineru_images"
                    if target.exists():
                        import shutil as _shutil
                        _shutil.rmtree(namespaced_images)  # target already present from previous run
                    else:
...
def _find_assets(inbox_dir: Path, asset_prefix: str, md_stem: str
    ...
    for candidate in [
        ...
        inbox_dir / f"{_safe_data_id(asset_prefix)}_images",  # fallback: MinerU used safe_data_id for folder name
        inbox_dir / "images",
    ]:
        if candidate.is_dir():
...
def _first_n_words(md_path: Path, n: int = 100) -> list[str]:
    """从 Markdown 文件中提取前 n 个词（小写，去除标点）。"""
    import re
    try:
...
def _text_duplicate_check(
    ctx: InboxCtx,
    *,
    top_words: int = 100,
    min_match: int = 50,
) -> str | None:
    """无 DOI 时，基于前 top_words 词的 LCS 相似度检测重复论文。

    扫描 data/papers/ 和 data/pending/ 中所有已有 paper.md，
    若与当前文件的前 top_words 词 LCS 长度 >= min_match，则认为是同一篇论文。

    Args:
        ctx: Inbox 上下文，需要 ``ctx.md_path`` 已设置。
        top_words: 比较的前 N 个词数量，默认 100。
        min_match: LCS 命中阈值，默认 50。

    Returns:
        重复论文的目录名（str），无重复时返回 None。
    """
    if not ctx.md_path or not ctx.md_path.exists():
        return None

    words_new = _first_n_words(ctx.md_path, top_words)
    if not words_new:
        return None

    papers_dir = ctx.cfg._root / "data" / "papers"
    pending_dir = ctx.pending_dir or ctx.cfg._root / "data" / "pending"

    candidates: list[tuple[Path, str]] = []
    for d in papers_dir.iterdir() if papers_dir.exists() else []:
        md = d / "paper.md"
        if md.exists():
            candidates.append((md, d.name))
    for d in pending_dir.iterdir() if pending_dir.exists() else []:
        md = d / "paper.md"
        if md.exists():
            candidates.append((md, f"pending/{d.name}"))

    for md_path, label in candidates:
        words_existing = _first_n_words(md_path, top_words)
        if not words_existing:
            continue
        lcs = _lcs_length(words_new, words_existing)
        if lcs >= min_match:
            _log.debug("text-dedup hit: LCS=%d >= %d -> %s", lcs, min_match, label)
            return label
