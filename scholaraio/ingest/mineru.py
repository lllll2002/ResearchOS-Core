...
def _safe_data_id(stem: str, max_bytes: int = 128) -> str:
    """确保 data_id 的 UTF-8 字节数不超过 MinerU API 的 128 字节限制。

    先将非 ASCII 字符替换为下划线（避免多字节字符撑大字节数），
    再在需要时截断至 max_bytes-9 字节并附加 8 位 hash 后缀保证唯一性。
    总字节数：(max_bytes-9) + 1("_") + 8(hash) = max_bytes。
    """
    import re
    # Replace non-ASCII characters with underscore to avoid multi-byte issues
    safe = re.sub(r'[^\x00-\x7F]', '_', stem)
    suffix_len = 9  # "_" + 8-char hash
    if len(safe.encode("utf-8")) <= max_bytes:
        return safe
    suffix = hashlib.md5(stem.encode("utf-8")).hexdigest()[:8]
    # Truncate to max_bytes - suffix_len bytes
    truncated = safe.encode("utf-8")[: max_bytes - suffix_len].decode("utf-8", errors="ignore")
...
# Route through local VPN/proxy so HTTPS requests reach mineru.net correctly
_CLOUD_PROXIES = {"http": "http://127.0.0.1:10080", "https": "http://127.0.0.1:10080"}
...
def convert_pdf_cloud(
    ...
    *,
    ...
    data_id = _safe_data_id(pdf_path.stem)
    payload: dict = {
...
def _convert_chunk_cloud(
    ...
    *,
    ...
    def _upload_one(idx: int) -> str | None:
        ...
        try:
            with open(pdf_path, "rb") as f:
                put_resp = requests.put(url, data=f, timeout=120, verify=False, proxies=_CLOUD_PROXIES)
            if put_resp.status_code not in (200, 201):
...
def _flatten_assets(src_dir: Path, out_dir: Path, data_id: str) -> None:
    ...
    for f in src_dir.iterdir():
        if f.is_file():
            ...
            if dest.exists():
                f.unlink()  # dest already present from a previous run, discard duplicate
            else:
                f.rename(dest)
    ...
    if zip_url:
        try:
            ...
            resp = requests.get(zip_url, timeout=120,
                                proxies=_CLOUD_PROXIES, verify=False)
            if resp.status_code == 200:
                md_content = None
