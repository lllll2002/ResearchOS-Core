...
命令：
    ...
    scholaraio note <paper-id> [--file notes.md] [--show]
...
def cmd_show(args: argparse.Namespace, cfg) -> None:
    from scholaraio.loader import load_l1, load_l2, load_l3, load_l4, load_notes
...
def _print_notes_if_exist(paper_d) -> None:
    from scholaraio.loader import load_notes
...
def cmd_note(args: argparse.Namespace, cfg) -> None:
    from scholaraio.loader import load_notes, append_notes
    ...
    if args.show or not args.file:
        notes = load_notes(paper_d)
        if notes:
            ui(f"[{paper_d.name}] 深读笔记：\n")
            ui(notes)
        else:
            ui(f"[{paper_d.name}] 暂无深读笔记。")
        return

    content = args.file.read()
    notes_path = append_notes(paper_d, content)
    ui(f"笔记已写入：{notes_path}")
...
def cmd_chunks(args: argparse.Namespace, cfg) -> None:
    try:
        from scholaraio.vectors import build_chunks
    except ImportError as e:
        _check_import_error(e)
    ...
    action = "Rebuilding" if args.rebuild else "Updating"
    ui(f"{action} section chunks: {papers_dir} -> {cfg.index_db}")
    count = build_chunks(papers_dir, cfg.index_db, rebuild=args.rebuild, cfg=cfg)
    label = "total" if args.rebuild else "new"
    ui(f"Done, {count} {label} chunks.")


def cmd_csearch(args: argparse.Namespace, cfg) -> None:
    try:
        from scholaraio.vectors import csearch
    except ImportError as e:
        _check_import_error(e)
    ...
    results = csearch(
        ...
        cfg=cfg,
        section_type=getattr(args, "section", None),
    )

    if not results:
        ui(f'No chunk results for "{query}".')
        return

    ui(f'Section search: "{query}"  {len(results)} results\n')
    for i, r in enumerate(results, start=1):
        dn = r.get("dir_name") or r["paper_id"][:12]
        st = r.get("section_type", "?")
        score = r.get("score", 0.0)
        snippet = r.get("snippet", "")[:120].replace("\n", " ")
        ui(f"[{i}] {dn}  ({st} {score:.3f})")
        ui(f"     {r.get('section_title', '?')}")
        ui(f"     {snippet}...")
        ui()
...
def cmd_usearch(args: argparse.Namespace, cfg) -> None:
    ...
    label = "Unified search (reranked)" if reranked else "Unified search"
    ui(f'{label}: "{query}"  {len(results)} results\n')
    for i, r in enumerate(results, start=1):
        match = r.get("match", "?")
        if reranked:
            extra = f"{match} rerank={r.get('rerank_score', 0.0):.3f}"
        else:
            extra = f"{match} {r.get('score', 0.0):.3f}"
        _print_search_result(i, r, extra=extra)


def cmd_ask(args: argparse.Namespace, cfg) -> None:
    from scholaraio.nlquery import classify_intent

    question = " ".join(args.query)
    ui(f'Question: "{question}"\n')

    intent = classify_intent(question, cfg)
    ui(f"Intent: {intent}\n")

    if intent == "search":
        # Route to unified search
        from scholaraio.index import unified_search
        results = unified_search(
            question, cfg.index_db,
            top_k=_resolve_top(args, cfg.search.top_k),
            cfg=cfg,
        )
        if not results:
            ...
            return
        reranked = any("rerank_score" in r for r in results)
        label = "Search (reranked)" if reranked else "Search"
        ui(f'{label}: {len(results)} results\n')
        for i, r in enumerate(results, start=1):
            match = r.get("match", "?")
            if reranked:
                extra = f"{match} rerank={r.get('rerank_score', 0.0):.3f}"
        ...
        try:
            result = nl_search(question, cfg.index_db, cfg=cfg, top_k=args.top)
        except RuntimeError as exc:
            ui(f"Error: {exc}")
            return
        ui(f"SQL: {result['sql']}")
        if result.get("retries"):
            ui(f"(auto-repaired {result['retries']}x)")
        ui()
        rows = result["results"]
        if not rows:
            ui("No results.")
        ...
        if "title" in columns and "paper_id" in columns:
            ui(f"{len(rows)} results\n")
            for i, r in enumerate(rows, 1):
                cite = r.get("citation_count", "")
                cite_suffix = f"  [cited: {cite}]" if cite else ""
                authors = r.get("authors", "")
                author_short = authors.split(",")[0].strip() + (" et al." if "," in authors else "") if authors else ""
                ui(f"[{i}] {r.get('title', '?')}")
                ui(f"     {author_short} | {r.get('year', '?')} | {r.get('journal', '?')}{cite_suffix}")
                ui()
        else:
            for row in rows:
                ui("  ".join(f"{k}={v}" for k, v in row.items()))

    elif intent == "graph":
        # Route to knowledge graph
        from scholaraio.kgraph import load_graph
        kg_path = cfg.papers_dir.parent / "kg.json"
        if not kg_path.exists():
            ui("Knowledge graph not built. Run: scholaraio kg build")
            return
        graph = load_graph(kg_path)
        hits = graph.search(question)
        if hits:
            ui(f"Found {len(hits)} entities:\n")
            for h in hits[:10]:
                ui(f"  [{h['type']}] {h['id']}: {h['label']}")
                r = graph.query(h["id"])
                for rel in r["outgoing"][:3]:
                    ui(f"    --{rel['relation']}--> {rel['target']}")
                for rel in r["incoming"][:3]:
                    ui(f"    <--{rel['relation']}-- {rel['source']}")
                ui()
        else:
            # Try extracting an entity ID from the question
            import re as _re
            m = _re.search(r"(C-\d{3}|EXP-\d{3}|BC-\d{3})", question)
            if m:
                r = graph.query(m.group(1))
                if r["entity"]:
                    e = r["entity"]
                    ui(f"[{e['type']}] {e['id']}: {e['label']}")
                    for rel in r["outgoing"]:
                        ui(f"  --{rel['relation']}--> {rel['target']}")
                    for rel in r["incoming"]:
                        ui(f"  <--{rel['relation']}-- {rel['source']}")
                else:
                    ui("No matching entity found.")
            else:
                ui("No matching entity found. Try: scholaraio kg search <keyword>")


def cmd_benchmark(args: argparse.Namespace, cfg) -> None:
    from scholaraio.benchmark import run_benchmark, save_benchmark

    ui("Running retrieval benchmark (5 queries)...\n")
    report = run_benchmark(cfg.index_db, cfg)

    ui(f"Overall score: {report['overall_score']:.0%} ({report['total_pass']}/{report['total_propositions']} propositions)\n")
    for q in report["queries"]:
        status = "PASS" if q["score"] >= 1.0 else "WARN" if q["score"] >= 0.5 else "FAIL"
        failed = [p["proposition"][:30] for p in q["propositions"] if not p["passed"]]
        detail = f" (missed: {', '.join(failed)})" if failed else ""
        ui(f"  [{status}] {q['id']}: {q['score']:.0%} — {q['query'][:50]}{detail}")

    log_path = save_benchmark(report, cfg.papers_dir.parent)
    ui(f"\nSaved to: {log_path}")

    if report["overall_score"] < 0.8:
        ui("\nWARNING: Score below 80% -- consider rebuilding index or checking embedding quality.")


def cmd_summarize(args: argparse.Namespace, cfg) -> None:
    from scholaraio.summarize import summarize_workspace

    query = " ".join(args.query)
    ws = args.ws
    ui(f'Recursive summarize: ws={ws}, query="{query}", batch={args.batch}\n')

    try:
        result = summarize_workspace(ws, query, cfg, batch_size=args.batch,
                                     use_local=getattr(args, "local", False),
                                     local_model=getattr(args, "model", "qwen3:8b"))
    ...
    ui(f"Papers: {result['paper_count']} | Batches: {result['batch_count']}\n")
    ui("=" * 60)
    ui(result["final_summary"])
    ui("=" * 60)


def cmd_nlsearch(args: argparse.Namespace, cfg) -> None:
    from scholaraio.nlquery import nl_search

    question = " ".join(args.query)
    ui(f'NL query: "{question}"\n')
    ...
    columns = result["columns"]
    # Paper-style results (have paper_id + title)
    if "title" in columns and "paper_id" in columns:
        ui(f"{len(rows)} results\n")
        for i, r in enumerate(rows, 1):
            cite = r.get("citation_count", "")
        ...
        # Aggregate / custom results — tabular output
        ui(f"{len(rows)} rows\n")
        for row in rows:
            ui("  ".join(f"{k}={v}" for k, v in row.items()))


def cmd_kg(args: argparse.Namespace, cfg) -> None:
    from scholaraio.kgraph import build_graph, save_graph, load_graph

    kg_path = cfg.papers_dir.parent / "kg.json"
    vault_path = getattr(args, "vault", None)

    # Auto-detect vault path: walk up from CWD or papers_dir until we find .obsidian
    if vault_path is None:
        for start in (Path.cwd(), cfg.papers_dir.resolve()):
            p = start
            while p != p.parent:
    ...
    if action == "build":
        ui(f"Scanning vault: {vault_path}\n")
        graph = build_graph(vault_path)

        if getattr(args, "llm", False):
            from scholaraio.kgraph import enrich_graph_llm
            max_p = getattr(args, "max_papers", 50)
            ui(f"\nLLM entity extraction (max {max_p} papers)...")
            added = enrich_graph_llm(graph, cfg.papers_dir, cfg, max_papers=max_p)
            ui(f"LLM added {added} entities")

        save_graph(graph, kg_path)
        s = graph.stats()
        ui(f"KG built: {s['total_entities']} entities, {s['total_relations']} relations")
        ui(f"Entity types: {s['entity_types']}")
        ui(f"Relation types: {s['relation_types']}")
        ui(f"Saved to: {kg_path}")

        # Auto-generate visualization
        try:
            from scholaraio.kgraph import visualize
            viz_path = kg_path.with_suffix(".html")
            visualize(graph, viz_path)
            ui(f"Visualization: {viz_path}")
        except ImportError:
            ui("(pyvis not installed, skipping visualization. pip install pyvis)")

    elif action == "query":
        if not kg_path.exists():
            ui("KG not built. Run: scholaraio kg build")
            return
        graph = load_graph(kg_path)
        result = graph.query(args.entity_id)
        if result["entity"] is None:
            ui(f"Entity not found: {args.entity_id}")
            hits = graph.search(args.entity_id)
            if hits:
                ui(f"\nSimilar entities:")
                for h in hits[:5]:
                    ui(f"  [{h['type']}] {h['id']}: {h['label']}")
        ...
        if not kg_path.exists():
            ...
            return
        graph = load_graph(kg_path)
        hits = graph.search(args.keyword)
        if not hits:
            ui(f"No match: {args.keyword}")
            return
        ui(f"Found {len(hits)} entities:\n")
        for h in hits:
            ui(f"  [{h['type']}] {h['id']}: {h['label']}  ({h['source_file']})")
        ...
        graph = load_graph(kg_path)
        s = graph.stats()
        ui(f"Total entities: {s['total_entities']}")
        ui(f"Total relations: {s['total_relations']}")
        ui(f"\nEntity type distribution:")
        for t, c in sorted(s["entity_types"].items(), key=lambda x: -x[1]):
            ui(f"  {t}: {c}")
        ui(f"\nRelation type distribution:")
        for t, c in sorted(s["relation_types"].items(), key=lambda x: -x[1]):
            ui(f"  {t}: {c}")

    elif action == "viz":
        viz_path = kg_path.with_suffix(".html")
        if not viz_path.exists():
            ui("Run 'scholaraio kg build' first (auto-generates visualization)")
            return
        import webbrowser
        webbrowser.open(str(viz_path))
        ui(f"Opened in browser: {viz_path}")

    else:
        ui("Usage: scholaraio kg [build|query|search|stats|viz]")
...
def _auto_export_library(model, cfg) -> None:
    """Auto-export topic model to browsable Obsidian library."""
    try:
        from scholaraio.topics import export_library
        lib_dir = cfg.papers_dir.parent.parent / "文献库"
        if not lib_dir.exists():
            # Try vault-level path
            for p in (Path.cwd(), cfg.papers_dir.resolve()):
                cur = p
                while cur != cur.parent:
                    if (cur / ".obsidian").exists():
                        lib_dir = cur / "Scholaraio" / "文献库"
                        break
                    cur = cur.parent
                if lib_dir.exists():
                    break
        n = export_library(model, lib_dir, cfg=cfg)
        ui(f"Library exported: {n} topic folders -> {lib_dir}")
...
def cmd_topics(args: argparse.Namespace, cfg) -> None:
    ...
    if args.build or args.rebuild:
        ...
        # Auto-export library after build
        _auto_export_library(model, cfg)
    else:
        try:
    ...
    # Export library
    if getattr(args, "export_lib", False):
        _auto_export_library(model, cfg)
        return
...
def cmd_classify(args: argparse.Namespace, cfg) -> None:
    import json
    from collections import defaultdict
    from pathlib import Path

    REVIEW_KEYWORDS = [
        'review', 'survey', 'overview', 'perspective', 'progress', 'advances',
        'state of the art', 'state-of-the-art', 'recent advances', 'emerging',
        'landscape', 'tutorial', 'introduction to', 'roadmap', 'comprehensive',
        'systematic review',
    ]
    REVIEW_JOURNALS = [
        'nature reviews', 'chemical reviews', 'chemical society reviews',
        'advanced reviews', 'wiley interdiscip', 'acm computing surveys',
        'ieee reviews', 'annual review', 'progress in', 'frontiers in',
        'trends in', 'current opinion',
    ]
    COMMENT_KEYWORDS = [
        'comment', 'editorial', 'letter', 'correspondence', 'news', 'viewpoint',
        'opinion', 'commentary',
    ]
    PREPRINT_JOURNALS = ['arxiv', 'biorxiv', 'medrxiv', 'preprint', 'ssrn']
    CONFERENCE_KEYWORDS = [
        'proceedings', 'conference', 'workshop', 'symposium', 'icml', 'neurips',
        'iclr', 'aaai', 'cvpr', 'acl', 'emnlp',
    ]

    import sqlite3
    db = sqlite3.connect(str(cfg.index_db))
    rows = db.execute('SELECT id, dir_name FROM papers_registry').fetchall()

    counts: dict = defaultdict(list)
    for pid, dir_name in rows:
        meta_file = Path(cfg.papers_dir) / dir_name / 'meta.json'
        if not meta_file.exists():
            counts['Unknown'].append(dir_name)
            continue
        m = json.loads(meta_file.read_text(encoding='utf-8'))
        title = (m.get('title') or '').lower()
        journal = (m.get('journal') or '').lower()

        if any(k in journal for k in PREPRINT_JOURNALS):
            counts['Preprint'].append(dir_name)
        elif any(k in journal for k in CONFERENCE_KEYWORDS):
            counts['Conference'].append(dir_name)
        elif any(k in title for k in COMMENT_KEYWORDS):
            counts['Comment/Editorial'].append(dir_name)
        elif (any(k in title for k in REVIEW_KEYWORDS) or
              any(k in journal for k in REVIEW_JOURNALS)):
            counts['Review/Survey'].append(dir_name)
        else:
            counts['Research Article'].append(dir_name)

    total = sum(len(v) for v in counts.values())
    order = ['Research Article', 'Review/Survey', 'Preprint', 'Conference', 'Comment/Editorial', 'Unknown']

    ui(f"\n论文类型分类：共 {total} 篇\n")
    for ptype in order:
        n = len(counts.get(ptype, []))
        if n:
            ui(f"  {ptype}: {n} 篇")
    ...
    out_path = Path(args.output) if args.output else cfg.papers_dir.parent / 'paper_types.txt'
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(f'Paper type classification: {total} papers\n\n')
        for ptype in order:
            papers = counts.get(ptype, [])
            if not papers:
                continue
            f.write(f'{"=" * 65}\n{ptype} ({len(papers)} papers)\n{"=" * 65}\n')
...
def cmd_draw(args: argparse.Namespace, cfg) -> None:
    from scholaraio.image_gen import generate_image

    prompt = " ".join(args.prompt)
    output_path = Path(args.output) if args.output else None

    # Override model/format if specified
    if args.model:
        cfg.image_gen.model = args.model
    if args.format:
        cfg.image_gen.format = args.format

    aspect = getattr(args, "aspect", None)

    api_key = cfg.image_gen.api_key or cfg.resolved_api_key()
    if not api_key:
        ui("错误：未配置图片生成 API key。请在 config.local.yaml 中设置 image_gen.api_key，"
           "或设置环境变量 VECTORENGINE_API_KEY")
        sys.exit(1)

    ui(f"正在生成图片...")
    ui(f"  提示词: {prompt}")
    ui(f"  格式  : {cfg.image_gen.format} | 模型: {cfg.image_gen.model or '（自动）'}")

    try:
        saved_path = generate_image(prompt, cfg, output_path=output_path, aspect_ratio=aspect)
    except Exception as e:
        _log.error("图片生成失败: %s", e)
        sys.exit(1)
...
def main() -> None:
    ...
    # --- note ---
    p_note = sub.add_parser("note", help="查看或写入论文深读笔记（notes.md）")
    p_note.set_defaults(func=cmd_note)
    p_note.add_argument("paper_id", help="论文目录名")
    p_note.add_argument(
        "--file", type=argparse.FileType("r", encoding="utf-8"),
        metavar="FILE", default=None,
        help="从文件读取笔记内容并追加到 notes.md（不指定则显示现有笔记）",
    )
    p_note.add_argument("--show", action="store_true", help="显示现有笔记（默认行为）")
    ...
    # --- chunks ---
    p_chunks = sub.add_parser("chunks", help="构建段落级分块向量（基于 TOC 切分论文全文）")
    p_chunks.set_defaults(func=cmd_chunks)
    p_chunks.add_argument("--rebuild", action="store_true", help="清空后重建")

    # --- csearch ---
    p_csearch = sub.add_parser("csearch", help="段落级语义检索（搜索论文具体章节）")
    p_csearch.set_defaults(func=cmd_csearch)
    p_csearch.add_argument("query", nargs="+", help="检索词")
    p_csearch.add_argument("--top", type=int, default=None, help="最多返回 N 条")
    p_csearch.add_argument("--section", type=str, default=None,
                           help="章节类型过滤: introduction/methods/results/discussion/conclusion")
    ...
    p_usearch.add_argument("--no-rerank", action="store_true", dest="no_rerank", help="跳过 reranker 重排序")
    _add_filter_args(p_usearch)

    # --- nlsearch (natural language → SQL) ---
    p_nl = sub.add_parser("nlsearch", help="Natural language metadata query (Text-to-SQL)")
    p_nl.set_defaults(func=cmd_nlsearch)
    p_nl.add_argument("query", nargs="+", help="NL question, e.g. 'papers after 2023 with citations > 50'")
    p_nl.add_argument("--top", type=int, default=None, help="Max results")

    # --- ask (intent-routed unified entry) ---
    p_ask = sub.add_parser("ask", help="Smart Q&A (auto-routes to search / data query / knowledge graph)")
    p_ask.set_defaults(func=cmd_ask)
    p_ask.add_argument("query", nargs="+", help="Natural language question")
    p_ask.add_argument("--top", type=int, default=None, help="Max results")

    # --- kg (knowledge graph) ---
    p_kg = sub.add_parser("kg", help="Knowledge graph: build, query, search, visualize")
    p_kg.set_defaults(func=cmd_kg)
    p_kg_sub = p_kg.add_subparsers(dest="kg_action")
    p_kg_build = p_kg_sub.add_parser("build", help="Scan vault and build knowledge graph")
    p_kg_build.add_argument("--vault", type=str, default=None, help="Vault path (auto-detected by default)")
    p_kg_build.add_argument("--llm", action="store_true", help="Use LLM to extract entities from papers (costs API tokens)")
    p_kg_build.add_argument("--max-papers", type=int, default=50, help="Max papers for LLM extraction (default 50)")
    p_kg_query = p_kg_sub.add_parser("query", help="Query entity relationships")
    p_kg_query.add_argument("entity_id", help="Entity ID (e.g. C-003, EXP-002, G3BP1)")
    p_kg_search = p_kg_sub.add_parser("search", help="Search entities")
    p_kg_search.add_argument("keyword", help="Search keyword")
    p_kg_stats = p_kg_sub.add_parser("stats", help="Graph statistics")
    p_kg_viz = p_kg_sub.add_parser("viz", help="Open interactive graph visualization in browser")

    # --- summarize (recursive map-reduce) ---
    p_sum = sub.add_parser("summarize", help="Recursive map-reduce literature synthesis (handles large workspaces)")
    p_sum.set_defaults(func=cmd_summarize)
    p_sum.add_argument("query", nargs="+", help="Research question")
    p_sum.add_argument("--ws", type=str, required=True, help="Workspace name")
    p_sum.add_argument("--batch", type=int, default=8, help="Papers per batch (default 8)")
    p_sum.add_argument("--local", action="store_true", help="Use local Ollama model (free, slower)")
    p_sum.add_argument("--model", type=str, default="qwen3:8b", help="Ollama 模型名（默认 qwen3:8b）")

    # --- benchmark ---
    p_bench = sub.add_parser("benchmark", help="Run retrieval quality benchmark (5 test queries)")
    ...
    p_topics.add_argument("--export", action="store_true", dest="export_lib", help="Export topics to Obsidian literature library")
    ...
    # --- classify ---
    p_classify = sub.add_parser("classify", help="按论文类型分类（综述/研究型/预印本等）")
    p_classify.set_defaults(func=cmd_classify)
    p_classify.add_argument("--output", default=None, help="输出文件路径（默认 data/paper_types.txt）")
    p_classify.add_argument("--summary", action="store_true", help="仅显示汇总，不列出具体论文")
    ...
    # --- draw ---
    p_draw = sub.add_parser("draw", help="调用 Gemini 生成图片（via VectorEngine）")
    p_draw.set_defaults(func=cmd_draw)
    p_draw.add_argument("prompt", nargs="+", help="图片描述（提示词）")
    p_draw.add_argument("--output", "-o", default=None, metavar="FILE",
                        help="保存路径（默认 workspace/images/<时间戳>.png）")
    p_draw.add_argument("--model", default=None, metavar="MODEL",
                        help="覆盖模型名（如 gemini-2.5-flash-image）")
    p_draw.add_argument("--format", default=None, choices=["native", "chat"],
                        help="调用格式：native（默认）| chat")
    p_draw.add_argument("--aspect", default=None, metavar="RATIO",
                        help="宽高比（仅 native 格式）：1:1 / 4:3 / 16:9 / 9:16 / 3:4")
