...
class RerankerConfig:
    ...
    Attributes:
        enabled: 是否在融合检索后启用 reranker 重排序。
        model: CrossEncoder 模型名称或 HuggingFace ID。
        device: 推理设备，``"auto"`` | ``"cpu"`` | ``"cuda"``。
        top_k: rerank 后最终返回条数。为 ``0`` 时沿用检索阶段的 top_k。
        source: 模型下载源，``"modelscope"`` | ``"huggingface"``。
    """
    enabled: bool = True
...
class ImageGenConfig:
    """图片生成后端配置（Gemini via VectorEngine 或其他 OpenAI 兼容服务）。

    Attributes:
        base_url: API 基础 URL，如 ``"https://api.vectorengine.ai"``。
        model: 模型名称。native 格式默认 ``"gemini-2.5-flash-image"``；
            chat 格式默认 ``"gemini-2.0-flash-exp-image-generation"``。
            留空则由 image_gen.py 按 format 自动选择。
        api_key: API 密钥，建议放 config.local.yaml 或环境变量。
        output_dir: 生成图片的保存目录（相对于项目根目录）。
        timeout: 请求超时（秒），图片生成通常需要较长时间。
        format: 调用格式，``"native"``（Gemini 原生）或 ``"chat"``（OpenAI 兼容）。
    """
...
def _build_config(data: dict, root: Path) -> Config:
    ...
    image_gen_data = data.get("image_gen", {}) or {}
    image_gen = ImageGenConfig(
        base_url=image_gen_data.get("base_url", "https://api.vectorengine.ai"),
        model=image_gen_data.get("model") or "",
        api_key=image_gen_data.get("api_key") or "",
        output_dir=image_gen_data.get("output_dir", "workspace/images"),
        timeout=int(image_gen_data.get("timeout", 120)),
        format=image_gen_data.get("format", "native"),
    )
    ...
    reranker = RerankerConfig(
        ...
        device=reranker_data.get("device", "auto"),
        top_k=int(reranker_data.get("top_k", 0)),
        source=reranker_data.get("source", "modelscope"),
    )

    return Config(paths=paths, llm=llm, ingest=ingest, embed=embed, search=search, topics=topics, log=log, zotero=zotero, image_gen=image_gen, reranker=reranker, _root=root)
