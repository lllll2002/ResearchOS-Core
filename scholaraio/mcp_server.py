...
def import_endnote(
    ...
    try:
        ...
        return json.dumps({"status": "ok", **stats, "dry_run": dry_run})
    except Exception as e:
...
# ============================================================================
#  Natural language & knowledge graph tools
# ============================================================================
...
def nl_search(
    ...
    """Natural language metadata query (Text-to-SQL).

    Converts a natural language question into SQL and executes it against the
    paper database. Supports year, citation count, journal, author filters.
    Auto-repairs SQL on failure (up to 2 retries).

    Examples: "2023年后引用量>50的相分离论文", "有多少篇论文", "Nature上的综述"
    """
    try:
        from scholaraio.nlquery import nl_search as _nl

        cfg = _get_cfg()
        result = _nl(question, cfg.index_db, cfg=cfg, top_k=top_k)
        return json.dumps(result, ensure_ascii=False)
    except RuntimeError as e:
        return _error("query_failed", str(e))
    except Exception as e:
        _log.exception("nl_search failed")
        return _error("internal", str(e))
...
def smart_ask(
    ...
    """Intent-routed unified Q&A.

    Automatically classifies the question into one of three intents:
    - "search": routes to hybrid search (usearch)
    - "data": routes to Text-to-SQL (nlsearch)
    - "graph": routes to knowledge graph query

    Returns the intent classification and search results.
    """
    try:
        ...
        if intent == "search":
            from scholaraio.index import unified_search as _usearch
            hits = _usearch(question, cfg.index_db, top_k=top_k, cfg=cfg)
            result["results"] = hits
        elif intent == "data":
            from scholaraio.nlquery import nl_search as _nl
            result.update(_nl(question, cfg.index_db, cfg=cfg, top_k=top_k))
        elif intent == "graph":
            ...
            if kg_path.exists():
                ...
                result["entities"] = hits[:10]
            else:
                result["error"] = "Knowledge graph not built. Run: scholaraio kg build"

        return json.dumps(result, ensure_ascii=False)
    except Exception as e:
        _log.exception("smart_ask failed")
        return _error("internal", str(e))
...
def kg_build(
    ...
    Scans wiki files, experiment records, and planning docs for entities
    (claims, experiments, paper references) and relationships (wikilinks,
    frontmatter relations).

    Args:
        use_llm: If True, also extract entities from paper abstracts/conclusions via LLM.
        max_papers: Maximum papers for LLM extraction (cost control).
    """
    try:
        from scholaraio.kgraph import build_graph, save_graph, visualize

        cfg = _get_cfg()

        # Auto-detect vault
        vault_path = cfg.papers_dir.resolve().parent.parent
        ...
        if use_llm:
            ...
            enrich_graph_llm(graph, cfg.papers_dir, cfg, max_papers=max_papers)

        kg_path = cfg.papers_dir.parent / "kg.json"
        save_graph(graph, kg_path)

        try:
            visualize(graph, kg_path.with_suffix(".html"))
        except ImportError:
            pass

        return json.dumps(graph.stats(), ensure_ascii=False)
    except Exception as e:
        _log.exception("kg_build failed")
        return _error("internal", str(e))


@mcp.tool()
def kg_query(entity_id: str) -> str:
    """Query knowledge graph for entity relationships.

    Returns all incoming and outgoing relations for the specified entity.

    Args:
        entity_id: Entity identifier (e.g., "C-003", "EXP-002").
    """
    try:
        from scholaraio.kgraph import load_graph
        ...
        if not kg_path.exists():
            return _error("not_found", "Knowledge graph not built. Run: scholaraio kg build")

        graph = load_graph(kg_path)
        result = graph.query(entity_id)
        return json.dumps(result, ensure_ascii=False)
    except Exception as e:
        _log.exception("kg_query failed")
        return _error("internal", str(e))


@mcp.tool()
def kg_search(keyword: str) -> str:
    """Search knowledge graph entities by keyword.

    Args:
        keyword: Search term to match against entity IDs and labels.
    """
    try:
        from scholaraio.kgraph import load_graph
        ...
        graph = load_graph(kg_path)
        hits = graph.search(keyword)
        return json.dumps(hits, ensure_ascii=False)
    except Exception as e:
        _log.exception("kg_search failed")
        return _error("internal", str(e))
