...
def load_notes(paper_dir: Path) -> str | None:
    ...
    Args:
        paper_dir: 论文目录路径（包含 meta.json 的那层目录）。
    ...
    if not notes_path.exists():
        return None
    return notes_path.read_text(encoding="utf-8", errors="replace").strip() or None


def append_notes(paper_dir: Path, content: str) -> Path:
    """向论文深读笔记追加内容。

    若 notes.md 不存在则创建；追加时自动插入分隔线。

    Args:
        paper_dir: 论文目录路径。
        content: 要追加的笔记内容（Markdown 文本）。
