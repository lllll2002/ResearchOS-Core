...
def _enrich_abstracts(results: list[dict], db_path: Path) -> None:
    """Fetch abstracts from FTS5 table and inject into result dicts (in-place)."""
    if not results:
        return
    ...
    try:
        ...
        rows = conn.execute(
            f"SELECT paper_id, abstract FROM papers WHERE paper_id IN ({placeholders})",
            pids,
        ).fetchall()
        pid_to_abs = {row[0]: row[1] for row in rows}
        for r in results:
            r["abstract"] = pid_to_abs.get(r["paper_id"], "")
    except Exception:
        pass
    finally:
        conn.close()
...
def unified_search(
    ...
    *,
    ...
    # -- Reranker (optional) --
    reranker_enabled = cfg is not None and cfg.reranker.enabled
    if reranker_enabled and len(results) > 1:
        try:
            from scholaraio.vectors import rerank
            # Fetch abstracts for richer reranking context
            _enrich_abstracts(results, db_path)
            rerank_top_k = cfg.reranker.top_k or top_k
            results = rerank(query, results, cfg=cfg, top_k=rerank_top_k)
        except (ImportError, Exception) as exc:
            import logging
            logging.getLogger(__name__).warning("reranker failed, using RRF order: %s", exc)
