...
def export_library(
    ...
    """Export topic model to a browsable Obsidian literature library.

    Generates one folder per topic with a README listing papers sorted by
    citation count, plus an index file with the topic overview.

    Args:
        model: BERTopic model with ``_metas`` and ``_topics`` attached.
        output_dir: Root directory for the library.
        labels: Optional dict mapping topic ID strings to labels.
        cfg: Config for LLM label generation (optional).

    Returns:
        Number of topic folders created.
    """
    import shutil
    from datetime import datetime

    metas_list = getattr(model, "_metas", [])
    topics_list = getattr(model, "_topics", [])
    if not metas_list or not topics_list:
        raise ValueError("Model has no paper metadata. Rebuild topics first.")

    if labels is None:
        label_file = Path(cfg.topics.model_dir if cfg else "data/topic_model") / "topic_labels.json"
        if label_file.exists():
            labels = json.loads(label_file.read_text("utf-8"))
        else:
            labels = {}

    if not labels and cfg:
        labels = _generate_labels_llm(model, cfg)

    topic_papers: dict[int, list[dict]] = {}
    for i, tid in enumerate(topics_list):
        topic_papers.setdefault(tid, []).append(metas_list[i])

    def _best_cite(m: dict) -> int:
    ...
    n_topics = len(topic_papers)
    n_papers = len(metas_list)

    idx = [
        f"---\ndate: {today}\ntags: [scholaraio, topics]\n---\n",
        "# Literature Library - Topic Overview\n",
        f"> Updated: {today}  ",
        f"> **{n_papers} papers**, **{n_topics} topics** (BERTopic + LLM labels)\n",
        "| # | Topic | Papers | Folder |",
        "|---|-------|--------|--------|",
    ]

    for tid in sorted(topic_papers.keys()):
        label = labels.get(str(tid), f"Topic {tid}")
        count = len(topic_papers[tid])
        folder = f"{tid:02d}-{label.replace(' ', '-')[:40]}"
        idx.append(f"| T{tid} | {label} | {count} | [[{folder}]] |")

        fp = output_dir / folder
        fp.mkdir(exist_ok=True)
        lines = [
            f"# T{tid}: {label}\n",
            f"**{count} papers**\n",
            "| # | Title | Year | Journal | Cited |",
            "|---|-------|------|---------|-------|",
        ]
        for j, m in enumerate(sorted(topic_papers[tid], key=lambda x: -_best_cite(x)), 1):
            t = (m.get("title") or "?")[:60]
            y = m.get("year") or "?"
            jr = (m.get("journal") or "?")[:25]
            c = _best_cite(m)
            lines.append(f"| {j} | {t} | {y} | {jr} | {c} |")

        (fp / "README.md").write_text("\n".join(lines), encoding="utf-8")

    idx_dir = output_dir / "00-索引"
    idx_dir.mkdir(exist_ok=True)
    (idx_dir / "知识库主题分布.md").write_text("\n".join(idx), encoding="utf-8")

    _log.info("Library exported: %d topics to %s", n_topics, output_dir)
    return n_topics


def _generate_labels_llm(model: "BERTopic", cfg: "Config") -> dict[str, str]:
    """Generate topic labels using LLM."""
    from scholaraio.metrics import call_llm

    info = model.get_topic_info()
    prompt = "Given these topic clusters, generate a concise academic label (5-10 words) for each.\n\nTopics:\n"
    for _, row in info.iterrows():
        tid = row["Topic"]
        words = model.get_topic(tid)
        kw = ", ".join([w for w, _ in (words or [])[:8]])
        prompt += f"Topic {tid} ({row['Count']} papers): {kw}\n"
    prompt += '\nOutput JSON: {"labels": {"0": "label", "1": "label", ...}}'

    result = call_llm(prompt, cfg, system="You are an academic taxonomy expert.",
                       json_mode=True, max_tokens=1000, purpose="topic_labeling")
    labels = json.loads(result.content).get("labels", {})

    label_path = Path(cfg.topics.model_dir) / "topic_labels.json"
    label_path.write_text(json.dumps(labels, indent=2, ensure_ascii=False), "utf-8")
    return labels
